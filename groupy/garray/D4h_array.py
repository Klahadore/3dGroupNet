...
class D4hArray(MatrixGArray):
    ...
    def mat2int(self, mat_data):
        ...
        data = np.zeros((input.shape[0], 3), dtype=np.int_)
    ...
    def int2mat(self, int_data):
        ...
        data = np.zeros((len(y),) + (3, 3), dtype=np.int_)
...
def rand(size=()):
    ...
    '''
    data = np.zeros(size + (3,), dtype=np.int_)
...
def identity(p='int'):
    ...
    e = D4hArray(data=np.array(li, dtype=np.int_), p='mat')
