...
class OhArray(MatrixGArray):
    ...
    def mat2int(self, mat_data):
        ...
        data = np.zeros((input.shape[0], 2), dtype=np.int_)
    ...
    def int2mat(self, int_data):
        ...
        data = np.zeros((len(index),) + (3, 3), dtype=np.int_)
    ...
    def get_mat(self, index, mirror):
        ...
        element = element.astype(dtype=np.int_)
        return element
...
def rand(size=()):
    ...
    '''
    data = np.zeros(size + (2,), dtype=np.int_)
...
def identity(p='int'):
    ...
    e = OhArray(data=np.array(li, dtype=np.int_), p='mat')
